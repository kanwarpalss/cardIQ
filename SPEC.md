...
| Issue | Status | Notes |
|---|---|---|
| Blinkit full-history live run | 🟡 KP ACTION | Exact copied cURL was proven to return Blinkit's own error page outside Chrome, so this is browser-context enforcement rather than a bad credential. `blinkit-browser-collector.ts` is implemented and syntax-verified; it safely transfers the copied request into the authenticated Blinkit console, where it pages all history and detail endpoints and downloads `blinkit-orders.json`. Dry-run then apply the importer; never share credentials. |
