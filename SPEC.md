...
## §6 Current State (as of 2026-08-29)
...
- **Mac Mini local-fallback copy made persistent (2026-08-23):** The `~/Code/cardIQ` checkout this section already describes as the launcher's local fallback is now kept running as `pm2` process `cardiq` (`npm run start`, port 3128), instead of only starting on-demand when the `cardiq` shell alias probes and finds nothing. The checkout there had drifted onto a stale local `master` branch (last synced 2026-05-16); switched it to track `main` and pulled current before building. `.env.local` copied over by hand (gitignored, not part of the pull). This does **not** change production — Vercel is still the real deploy target below; this only means the Mac-Mini-first fallback in the shared launcher now always has a warm copy to hand off to instead of a cold start.
- **Cross-machine launcher (2026-08-15):** Type `cardiq` in any terminal. It probes CardIQ on the Mac mini first through its stable Tailscale IP (`100.81.29.11:3128/login`) and opens that copy only when the page identifies itself as CardIQ. Only a genuinely unreachable mini triggers local fallback; an HTTP error or wrong service fails loudly instead of starting a duplicate. The canonical launcher is `~/AI HQ/System/shell/shared-aliases.sh`; the old iCloud-root `shared-aliases.sh` is now a compatibility bridge, so every already-wired Mac receives the same behavior. The same remote-first rule covers `wa`/`whatsapp`, `investment`, `alug`, `wealth`, and `nourish`; explicit `*-local` commands skip the remote probe.
- **Local fallback:** The local copy stays at `~/Code/cardIQ` on fixed port 3128. A working CardIQ server is opened immediately and is never fetched over, rebuilt, or killed by the launcher. With no server, clean checkouts update from GitHub when reachable; a GitHub outage uses the existing checkout, and local edits are preserved and built without pulling. Port conflicts and missing `.env.local` fail in plain English. Each Mac owns its private `.env.local`; it is never copied to GitHub or the shared launcher.
